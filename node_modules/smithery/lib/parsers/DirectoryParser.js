...
class DirectoryParser {
    parse(sFilePath, options) {
        ...
        if (stats.isDirectory()) {
            oToken.children = fs_1.readdirSync(route).filter((child) => {
                return !exclMatcher(path_1.join(route, child));
            }).map((child) => {
                return this.parse(path_1.join(route, child), { parent: oToken, exclude: options === null || options === void 0 ? void 0 : options.exclude });
            });
        }
        else {
