    ...
    _processFiles(aNodesToProcess, sTargetPath) {
        for (const oNode of aNodesToProcess) {
            if (oNode.type === enums_1.FileType.Folder) {
                if (oNode.name !== 'root' && !fs_1.existsSync(path_1.join(sTargetPath, oNode.name))) {
                    fs_1.mkdirSync(path_1.join(sTargetPath, oNode.name));
                }
                this._processFiles(oNode.children || [], path_1.join(sTargetPath, oNode.name !== 'root' ? oNode.name : ''));
            }
            else {
                //we will write the file even if no content is provided!
                if (!fs_1.existsSync(path_1.join(sTargetPath, oNode.name))) {
                    fs_1.writeFileSync(path_1.join(sTargetPath, oNode.name), (oNode === null || oNode === void 0 ? void 0 : oNode.content) || '');
                }
            }
        }
    }
