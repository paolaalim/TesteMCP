...
describe('New feature "exclude" test', () => {
    it('Try to exclude the "*.md" file', () => {
        ...
        p.build('config2');
        chai_1.expect(fs.existsSync('./build')).to.be.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'README.md'))).to.be.not.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'folder_feature1'))).to.be.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'folder_feature1', 'file_subFeature1_feature2.txt'))).to.be.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'file_feature2.txt'))).to.be.true;
        mock.restore();
    });
    it('Exclude the "folder_feature1" from the build process', () => {
        ...
        p.build('config2');
        chai_1.expect(fs.existsSync('./build')).to.be.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'README.md'))).to.be.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'folder_feature1'))).to.be.not.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'file_feature2.txt'))).to.be.true;
        mock.restore();
    });
    ...
    it('Expect a successfull build for all files, together with excluded files from the build at the build folder', () => {
        ...
        const p = new Project_1.Project();
        chai_1.expect(fs.existsSync('./build')).to.be.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'node_modules'))).to.be.true;
        p.build('config2');
        chai_1.expect(fs.existsSync('./build')).to.be.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'node_modules'))).to.be.true;
        chai_1.expect(fs.existsSync(path.join('./build', 'README.md'))).to.be.true;
...
//# sourceMappingURL=feature_exclude.spec.js.map